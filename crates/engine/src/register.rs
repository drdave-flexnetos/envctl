...
pub fn synth_dropin(spec: &RegisterSpec) -> String {
    ...
    // install script). `validate_spec_strings` only rejects `'''`, not `"""`.
    // Sanitize every interpolated field via `desc_safe` before embedding.
    s.push_str(&format!(
        "strategy={} source={} ref={} sha={sha} build_system={}\n",
        desc_safe(&spec.strategy_tag),
        desc_safe(&spec.source),
        desc_safe(&ref_disp),
...
/// AUDIT-FIX: collapse newlines/CR so a user-derived field stays on one `#`
/// comment line and cannot inject a top-level TOML key.
fn comment_safe(s: &str) -> String {
    s.replace(['\n', '\r'], " ")
}

/// AUDIT-FIX: sanitize a user-derived field for embedding inside a TOML `"""`
/// multi-line basic string. Strip `"` and `\` (so it can neither close the
/// string with `"""` nor start an escape) and flatten line breaks to spaces.
fn desc_safe(s: &str) -> String {
    s.chars()
        .map(|c| match c {
            '"' | '\\' => ' ',
            '\n' | '\r' => ' ',
