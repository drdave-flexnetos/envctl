...
Engine-integration cluster: kasetto verbs absorbed into `Engine::agent_{add,remove,lock,list,clean}`,
verified via NEW `crates/engine/tests/agent_command_parity.rs` (+22 tests). `cargo test -p envctl-engine`
**96 passed** (baseline 74); no-c PASS; fmt clean. **C-12/C-13 port kasetto's verbatim oracle tests**
(source_edit 6 split_at_ref vectors; init 2 path-resolution vectors — w/ envctl renames). **C-07..C-11,
C-14 have NO kasetto verb-level test oracle** (kasetto's verbs were 0-test) so proven by end-to-end Engine
integration over their (already-`[x]`) primitives: **C-07** add (ERR --ref+--branch / --locked-w/o-no-sync
/ @ref+--ref / already-exists; dry_run zero-writes), **C-08** remove (not-found-in-any-list / not-found-in-
section ERR; dry_run zero-writes), **C-09** lock `--check` drift-exit without writing + Locked zero-network,
**C-10** list kind+scope filter read-only, **C-11+C-14** clean preview-then-apply (fail-closed never-partial-
prune; tears down only lock-tracked; untracked `weave` survives = never-clobber; C-14 asset-cleanup folds
into clean), **C-13** init path-resolution business (template string is FE, not ported). Deviation #2:
no `agent_init_template` verb (init folds into add/sync per ledger) — certifiable business proven, no fake.
