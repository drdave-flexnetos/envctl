...
bring-up. This has now **landed** — `crates/secretd/src/main.rs` serves the control plane
(no `todo!()` scaffold) and answers `secretd --self-check` (a non-serving startup pre-flight).
The component therefore ships with `enable=true`; CI's `ci/gates/enable.sh` enforces the
post-Phase-6 invariant (main.rs not a scaffold AND, if enabled, the `--self-check` surface
exists) — §4.3, §7.3.
...
# visible) WITHOUT triggering a destructive fix.
[component.verify]
kind = "command"
command = "bash"
...
enable = true           # Phase 6 landed: secretd serves + answers --self-check (see §4.3 / §7.3).
content = '''
[Unit]
...
- `secretd --self-check` is the Phase-6 verify surface and now **exists** (`VERIFIED`:
  `crates/secretd/src/main.rs`, the `Cli`/`self_check` path) — a non-serving pre-flight that
  validates the crypto provider, XDG paths, and store config, then exits (0 = healthy). `--init`
  remains an intended future surface and is not yet wired. The verify hook still degrades safely: a
  misconfigured/un-built daemon returns non-zero → `verify=false`, no destructive fix triggered
  because `detect`, not health, is the install predicate.
...
Materialized as `ci/gates/enable.sh`. **Phase 6 has landed**, so the gate now asserts the *inverse*
of its pre-Phase-6 form (which kept `enable = false` while `main.rs` was a `todo!()` scaffold):
...
grep -q 'todo!("secretd server bring-up' crates/secretd/src/main.rs \
  && { echo "main.rs is still the scaffold — manifest must keep enable=false"; exit 1; }

# (2) If the unit ships enabled, the verify hook's `secretd --self-check` surface MUST exist
#     (in both the manifest and the source) — else an enabled unit wires a verify that can never
#     pass, and a bare `secretd` with no `--self-check` would hang the hook by serving forever.
if grep -Eq '^[[:space:]]*enable[[:space:]]*=[[:space:]]*true' manifest/env-ctl.toml; then
  grep -q -- 'secretd --self-check' manifest/env-ctl.toml || { echo "enabled but verify lacks --self-check"; exit 1; }
  grep -q -- 'self-check'           crates/secretd/src/main.rs || { echo "enabled but no --self-check subcommand"; exit 1; }
fi
```

The pre-Phase-6 scaffold guard is retired with this PR and `enable` flips to `true` in the same PR.
...
### 7.3 Why `enable = false` *was* held until Phase 6 (fail-closed deploy)

A pre-Phase-6 `secretd` was `todo!()`. Auto-enabling it would have put a panicking unit in a restart
loop and, worse, given operators a false sense that "the vault is up". Fail-closed: the component
installed the *capability* but did not start a daemon that could not enforce its own guards (FS-S9 —
a guard that cannot prove its precondition must not silently pass; the same principle applies at the
deploy layer). Now that Phase 6 has landed — `secretd` serves and `--self-check` lets `verify` probe
health without blocking — `enable = true`, and `ci/gates/enable.sh` (§4.3) keeps the invariant
honest: it refuses an enabled unit whose `--self-check` surface is missing.
