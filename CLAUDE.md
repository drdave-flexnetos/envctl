...
| 2026-06-05 | Add Option Y (orchestrator-owned grit done for intra-repo serialized merge) | skills/feature-forge; agents/{rust-implementer,invariant-guardian,continuity-steward}; skills/session-relay | Give real intra-repo multi-writer parallelism on shared files: grit does its serialized AST-lock rebase+merge, but the guardian gate is inverted to sit BETWEEN work and merge and only the orchestrator calls grit done (after PASS). Opt-in via FORGE_OPTION_Y; Option X / sequential remains the unchanged default. |
| 2026-06-05 | Full N-branch A2 continuity correctness — PR-4 | skills/{session-relay,forge-loop,feature-forge}; agents/continuity-steward | Make a cross-repo (A2, incl. Option-Y waves) cycle hand off + resume across sessions with zero loss / no deadlock for N repos·branches·_workspace/<repo>/ trees. One committed host HANDOFF.md enumerates N repos (full per-repo table); member durability = per-repo branch tip (no meta-commit at handoff). RESUME fans baseline out (meta --json git worktree exec --parallel cleanliness + per-repo gates), recreates a pruned set deterministically on recorded branches (--repo alias:branch, never bare), reconciles grit release-and-re-claim-fresh except gated-PASS-but-unmerged Option-Y writers, per-repo idempotent (tip vs last-good) so cron+human can't double-build. Single-repo path unchanged. |
